	...
	int ppMax, int offense, int defense, int iq)
{
	...
	this->inventory.addItem(Weapon(5));
	this->inventory.addItem(Armor(5));
	this->inventory.addItem(Weapon(5));
	this->inventory.addItem(Armor(5));
	this->inventory.addItem(Weapon(5));
...
void Character::equipItem(unsigned index)
{
	...
	else
	{
		...
		{
			std::cout << "Error. Item is not weapon or armor.";
		}
	}
}
...
std::string Character::getInventoryAsString()
{
	...
	{
		inv += std::to_string(i+1) + ": " + this->inventory[i].toString() + "\n";
	}
